...
class NetworkBuilder:
    ...
    def generate_routes(self, pddl_vehicle: PddlVehicle, info: VehicleInfo) -> Optional[FrozenSet[int]]:
        ...
        # assert(original_edges[0] not in self.sub_graph.road_network.edges)
        # assert(None not in self.sub_graph.road_network.get_edges(original_edges[1:-1]))
        ...
        assert(self.sub_graph.road_network.check_edge_sequence(pddl_vehicle.graph_route.edge_list))
    ...
    def prepare_graph(self, graph: Graph, sub_graph: Graph) -> Dict[str, Set[str]]:
        ...
        """
        print(f"Preparing sub-graph: '{sub_graph.road_network.map_name}' connections to original graph ...")
