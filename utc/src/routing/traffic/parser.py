...
class Parser:
    ...
    def process_result(self, episode: PddlEpisode) -> Optional[Dict[Element, Element]]:
        ...
        for vehicle_id, pddl_routes in episode.result.parse_result().items():
            ...
            if vehicle is None:
                print(f"Unable to find vehicle: {vehicle_id} in container!")
                print(f"In problem: {episode.problem.name}, result: {episode.result.name}")
    ...
    def create_route(self, vehicle: PddlVehicle, pddl_routes: List[int], network: RoadNetwork) -> Optional[Element]:
        ...
        for route in network.get_routes(pddl_routes):
            if route is None:
                # TODO check
                print(f"Unable to find route for vehicle: {vehicle.vehicle.id} on graph: {self.sub_graph.road_network.map_name}")
                return None
