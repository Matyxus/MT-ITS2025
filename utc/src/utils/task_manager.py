...
class TaskManager:
    ...
    def call_shell(command: str, timeout: float = None, cwd: str = None, message: bool = True) -> Tuple[bool, int]:
        ...
        """
        if message:
            print(f"Calling command: '{cmd_split(command, posix=False)}' with timeout: '{timeout}', cwd: {cwd}")
        ...
        try:
            proc = Popen(cmd_split(command, posix=False), stdout=DEVNULL, stdin=DEVNULL, cwd=cwd, encoding="utf-8")
