...
class SOLVERS:
    """
    Class defining solvers calls as format string (expected arguments are:
    "domain_file.pddl" "problem_file.pddl" "result_file.pddl")
    """
    MERCURY: str = (DirPaths.PDDL_SOLVERS.format("Mercury/plan-utc") + " {0} {1} {2}")
    MIP: str = ("python " + DirPaths.PDDL_SOLVERS.format("mip.py") + " {0} {1} {2}")

    @staticmethod
    def get_solver(solver_name: str) -> str:
        """
        ...
        """
        planer: str = getattr(SOLVERS, solver_name.upper(), "")
        if not planer:
            print(f"Planner: {solver_name} is not defined in SOLVERS!")
        return planer
