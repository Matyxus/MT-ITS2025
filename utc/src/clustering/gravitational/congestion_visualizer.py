...
def get_edge_stats(dump_file: str, edge_id: str) -> None:
    print(f"--------- {dump_file} ---------")
    xml_file: XmlFile = XmlFile(dump_file)
    total: float = 0
    counter: int = 0
    for i, interval in enumerate(xml_file.root.findall("interval")):
        for edge in interval.findall("edge"):
            if edge.attrib["id"] != edge_id:
                continue
            print(f"Interval: {i}, ci: {edge.attrib['congestionIndex']}, (entry/left): {edge.attrib['entered'], edge.attrib['left']}")
            if 'teleported' in edge.attrib:
                print(f"tp: {edge.attrib['teleported']}")
    #         diff = 1.0 - (float(edge.attrib["speedRelative"]) + round(float(edge.attrib['congestionIndex']), 3))
    #         total += diff
...
if __name__ == '__main__':
    ...
    data = [
        # DirPaths.SCENARIO.format("Base") + "/edgedata.out.xml"
        DirPaths.SCENARIO_STATISTICS.format("lust_25200_32400_routed_mip") + "/stats_edgeData.out.xml",
        DirPaths.SCENARIO_STATISTICS.format("lust_25200_32400") + "/stats_routed_edgeData.out.xml"
    ]
    array1 = visualizer.load_ci(data[0], graph, (25200, 26100))
    array2 = visualizer.load_ci(data[1], graph)
    # # print(np.where(array1 - array2 > 0.5))
    # get_edge_stats(data[0], "-31268#0")
    # get_edge_stats(data[1], "-31268#0")
    visualizer.plot_ci(array1, graph)
